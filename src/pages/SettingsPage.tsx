...
import { Download, Upload, Shield, Database, Store, Save, Printer, Globe, FileInput, Trash2, FileDown, RefreshCw, CheckCircle2, AlertCircle, Moon } from 'lucide-react';
...
const SettingsPage: React.FC = () => {
  ...
  // ✅ State untuk mengecek role user yang sedang login
  const currentUser = indexdbUser.getCurrentUser();
  const isSuperAdmin = indexdbUser.isSuperAdmin();
  ...
  // ✅ RESTORE DATABASE (timpa semua data dari file JSON) — menggunakan blob.text() (stabil di Android)
  const handleRestore = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!confirm('⚠️ Peringatan: Memulihkan data akan MENGHAPUS semua data produk dan transaksi saat ini, lalu menggantinya dengan data dari file. Lanjutkan?')) return;

    setLoadingKey('restore');
    
    try {
      // ✅ Gunakan blob.text() — lebih stabil di Android WebView
      ...
      if (success) {
        showStatus('success', '✅ Data berhasil dipulihkan!', 4000);
        setLoadingKey(null);
      } else {
  ...
  // ✅ IMPORT PRODUK MASSAL dari JSON — menggunakan blob.text() (lebih stabil di Android WebView)
  const handleImportProducts = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!confirm('Import produk akan menambahkan/menimpa produk dengan ID/SKU yang sama. Produk yang tidak ada di file TIDAK akan dihapus. Lanjutkan?')) return;

    setLoadingKey('importProducts');
    
    try {
      // ✅ Gunakan blob.text() — lebih modern, tidak pakai callback, tidak ada masalah session di Android
      const json = await file.text();
      let data: any;
      try {
      ...
      if (result.success > 0) {
        showStatus(
          'success',
          `✅ Berhasil import ${result.success} produk! (+${newProducts > 0 ? newProducts : 0} baru, ${result.success - (newProducts > 0 ? newProducts : 0)} diperbarui). Total: ${afterCount} produk.`,
          4000
        );
        setLoadingKey(null);
      } else {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm space-y-6 md:col-span-2">
              ...
              <div>
                ...
                {!isSuperAdmin && (
                  <p className="text-[9px] font-bold text-red-500 mt-1 uppercase tracking-wider">
                    ⚠️ Hanya Super Admin yang dapat mengelola akun
                  </p>
                )}
              </div>
            </div>
          ...
          {/* Form Tambah User Baru — hanya untuk Super Admin */}
          ...
          {/* Grid List User — hanya tampil untuk Super Admin */}
          {isSuperAdmin ? (
            ...
            <div className="p-8 bg-red-50 rounded-[28px] border border-red-100 text-center space-y-3">
              ...
              <p className="font-black text-red-600 text-sm">Akses Terbatas</p>
              <p className="text-[11px] font-medium text-red-500 leading-relaxed max-w-sm mx-auto">
                Hanya akun <b>Super Admin</b> yang dapat mengelola pengaturan akun pengguna. 
                Silakan login dengan akun Super Admin untuk mengakses fitur ini.
              </p>
              <p className="text-[10px] font-bold text-red-400 bg-red-100/50 px-3 py-2 rounded-xl inline-block">
