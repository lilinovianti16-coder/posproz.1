...
import type { DBTransaction, DBTransactionItem } from '@/lib/DatabaseService';
...
const HistoryPage: React.FC = () => {
  ...
  const loadTransactions = async () => {
    ...
    try {
      ...
      const data = db.getAllTransactions();
      // Transform ke format yang compatible dengan UI lama
      const mapped = data.map((t: DBTransaction) => ({
        id: t.id,
        total: t.total,
        subtotal: t.subtotal,
  ...
  const handleReprint = async (trxId: string) => {
    try {
      ...
      const items = db.getTransactionItems(trxId);
      if (!trx) {
        alert("Data transaksi tidak ditemukan.");
        return;
      }

      const trxSubtotal = trx.subtotal || items.reduce((acc: number, item: DBTransactionItem) => {
        return acc + (item.qty * item.price_at_sale);
      }, 0);
      const trxDiscount = trx.discount_amount || Math.max(0, trxSubtotal - trx.total);
      ...
      await printerService.printReceipt({
        ...
        items: items.map((item: DBTransactionItem) => ({
          product_name: item.product_name,
          qty: item.qty,
          price_at_sale: item.price_at_sale,
          price: item.price_at_sale,
          harga: item.price_at_sale,
          quantity: item.qty,
          jumlah: item.qty,
          harga_jual: item.price_at_sale
        })),
          ...
          <div className="divide-y divide-slate-50">
            {filteredTransactions.map((trx) => (
              <div 
                ...
                onClick={async () => {
                   await db.init();
                   const trxData = db.getTransactionById(trx.id);
                   const itemsData = db.getTransactionItems(trx.id);
                   setSelectedTrx({
                     ...trxData,
                     items: itemsData
                   });
